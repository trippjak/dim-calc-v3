...
function dwc_enqueue_scripts() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'dim_weight_calculator')) {
        // Load React from WordPress core
        wp_enqueue_script('wp-element');
        
        // Enqueue our bundled calculator
        wp_enqueue_script(
            'dim-weight-calculator',
            plugins_url('dist/calculator.js', __FILE__),
            ['wp-element'],
            '1.0.0',
            true
        );
        ...
        wp_enqueue_style(
            ...
            plugins_url('dist/calculator.css', __FILE__),
            [],
            '1.0.0'
        );
        ...
        wp_localize_script(
            'dim-weight-calculator',
            'dwcData',
            ['carriers' => dwc_get_carriers_data()]
        );
...
function dwc_init_script() {
    ...
    function initDimWeightCalculator() {
        ...
        if (container && window.wp && window.wp.element) {
            ...
            render(
                createElement(window.DimWeightCalculator.default, { 
                    carriers: window.dwcData.carriers 
                }),
                root
            );
        }
